...
@section('content')
    <div class="page-titles">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active"><a href="#">Add RFID Card!</a></li>
        </ol>
    </div>
    <div class="row">
        @if(count($list)>0)
                    ...
                    <div class="card-body tab-content pt-2">
                        @foreach($list as $key=>$data)
                            <div class="d-flex border-bottom flex-wrap pt-3 list-row align-items-center mb-2">
                                ...
                                @if(isset(Auth::user()->info->roleDetail->rolePermission->permissions['rfid']['delete']))
                                <div class="col-xl-2 col-xxl-4 col-lg-2 col-sm-4 activities mb-3 mr-auto pl-3 pr-3 text-sm-center">
                                    ...
                                    <form action="{{ route('init.remove') }}" method="post" style="display: none;" id="delFORM{{$key}}">@csrf
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $.each(data, function (key, value) {
                            ...
                            $('#ShowResult').empty().append('<tr><td>' + value.member_id + '</td><td>' + value.first_name + " " + value.last_name + '</td><td>'+ value.email +'</td><td><a href="/admin/access/rfid/'+ value.id +'" class="btn btn-sm btn-primary">Select</a></td></tr>');
                        })
                    }
                })
            });
